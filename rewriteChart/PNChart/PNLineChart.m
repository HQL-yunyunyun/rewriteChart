...
@property (nonatomic, strong) NSMutableArray *heightestPoints;
...
- (void)strokeChart {
    ...
    if (self.showInflexionHighestPoint) {
         [self calculateHeightestPointChartPath:_chartPath andPointsPath:_pointPath andPathKeyPoints:_pathPoints andPathStartEndPoints:_endPointsOfPath];
    } else {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        CGPoint heightestPoint = [self.heightestPoints[lineIndex] CGPointValue];
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(heightestPoint.x - 4, heightestPoint.y - 4, 8, 8)];
...
- (void)calculateHeightestPointChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath {
    // Draw each line
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        PNLineChartData *chartData = self.chartData[lineIndex];
        ...
        NSMutableArray *pointsArray2 = [NSMutableArray array];
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            [pointsArray addObject:[NSValue valueWithCGPoint:circleCenter]];
            [pointsArray2 addObject:[NSValue valueWithCGPoint:CGPointMake(x, y)]];
     
            if (maxValue < yValue) {
                maxValue = yValue;
                topPoint = circleCenter;
            }
            
            if (i == chartData.itemCount - 1) {
                for (NSInteger i = 0; i < pointsArray.count; i++ ) {
                    CGPoint heightPoint = [pointsArray[i] CGPointValue];
                    CGPoint point = [pointsArray2[i] CGPointValue];
                    CGFloat x = point.x;
                    CGFloat y = point.y;
                    if (heightPoint.x == topPoint.x && heightPoint.y == topPoint.y ) {
                        
                        [self.heightestPoints addObject:[NSValue valueWithCGPoint:heightPoint]];
                        
                        [pointPath moveToPoint:CGPointMake(heightPoint.x + (inflexionWidth / 2) + 1, heightPoint.y)];
                        [pointPath addArcWithCenter:heightPoint radius:inflexionWidth / 2 + 1 startAngle:0 endAngle:2 * M_PI clockwise:YES];
                        if (i > 0) {
                            // calculate the point for line
                            float distance = sqrt(pow(x - last_x - 1, 2 ) + pow(y - last_y - 1, 2 ));
                        ...
                        [pointPath moveToPoint:CGPointMake(heightPoint.x + (inflexionWidth / 2), heightPoint.y)];
                        [pointPath addArcWithCenter:heightPoint radius:inflexionWidth / 2 startAngle:0 endAngle:2 * M_PI clockwise:YES];
                        if (i > 0) {
                            // calculate the point for line
...
- (NSMutableArray *)heightestPoints {
    if (!_heightestPoints) {
        _heightestPoints = [NSMutableArray array];
    }
    return _heightestPoints;
}
